defmodule FamilyDashboard.WorkersTest do
  ...
  alias FamilyDashboard.{Dashboard, Workers.CalendarSync, Workers.WeatherRefresh}
  ...
  test "WeatherRefresh cancels when no location is configured" do
    {:ok, setting} = Dashboard.current_setting()
    Dashboard.update_setting!(setting, %{latitude: nil, longitude: nil})

    assert {:cancel, :no_location} = perform_job(WeatherRefresh, %{})
  end
